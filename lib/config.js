...
function readCommandLine() {
  return require("yargs")(process.argv.slice(2))
    ...
    .option("template", {
      alias: "t",
      describe: `Specify the template. Must reside in the ${TEMPLATES_PATH} folder. Specify --generator as well.`,
    })
    .option("config", {
      alias: "conf",
      describe: `Specify a configuration file that contains template and generator. It must be placed in ${CONFIG_PATH} 
            folder. If you specify a config file do not specify generator and template seperately. They will be ignored.
        `,
    })
    .option("generator", {
      alias: "g",
      describe: `Specify a generator of data. Must reside in ${GENERATORS_PATH} folder. Specify template as well (--template)`,
    })
    .option("startid", {
      alias: "s",
      default: 1,
    ...
    .implies("template", "generator")
    .implies("generator", "template")
